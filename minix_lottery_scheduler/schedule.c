...
PUBLIC int lottery(void){
	...
	winner = rand() % (totaltickets) +1; /* winning ticket*/
    
	 /* Move processes that already won to the idle queue*/
	for (proc_nr=0, rmp=schedproc; proc_nr < NR_PROCS; proc_nr++, rmp++) {
		if (rmp->flags & IN_USE) {
			if (rmp->priority == MAX_USER_Q) {
				rmp->priority = MIN_USER_Q;/*lower its priority*/
				rmp->time_slice = (unsigned) -1;
				/*if ((k = schedule_process(rmp) != OK) {}*/
			}
		}
	}

	/* pick a winning process */
	for (proc_nr=0, rmp=schedproc; proc_nr < NR_PROCS; proc_nr++, rmp++) {
		if (rmp->flags & IN_USE) {
			if (rmp->priority == MIN_USER_Q) {	
				ticketnum += rmp->tickets;
				if(ticketnum<= winner) {
					rmp->priority = MAX_USER_Q;
					winner-=rmp->tickets;
					k = schedule_process(rmp);
					break;
				}
			}		
		}
